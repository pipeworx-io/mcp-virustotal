...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'VirusTotal');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    case 'lookup_file':
      return lookup(apiKey, 'files', reqStr(args, 'hash', '"275a021bbfb6489e54d471899f7db9d1663fc695ec2fe2a2c4538aabf651fd0f" (SHA-256)').toLowerCase().trim());
    case 'lookup_url':
      return lookup(apiKey, 'urls', urlId(reqStr(args, 'url', '"https://example.com/path"')));
    case 'lookup_domain':
      return lookup(apiKey, 'domains', reqStr(args, 'domain', '"example.com"').trim().toLowerCase());
    case 'lookup_ip':
      return lookup(apiKey, 'ip_addresses', reqStr(args, 'ip', '"8.8.8.8"').trim());
    default:
...
async function lookup(apiKey: string, kind: string, id: string) {
  ...
  const res = await pwFetch(`${BASE_URL}/${kind}/${encodeURIComponent(id)}`, {
